...
def loss_function_continuous(recon_x, x, mu, logvar):
    ...
    LOGC = -compute_log_C(recon_x)
    
    return BCE + KLD + LOGC


def train(epoch, model, train_loader, loss_function, device, log_interval=10):
    model.train()
    train_loss = 0
...
def test(epoch, model, test_loader, loss_function, batch_size, device):
    model.eval()
    test_loss = 0
    with torch.no_grad():
